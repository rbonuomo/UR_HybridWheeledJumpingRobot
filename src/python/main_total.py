...
with open('results/'+folder+'/data_phase0.txt', 'w') as f:
...
with open('results/'+folder+'/data_phase0.txt', 'a') as f:
...
#plt.show()
# with open('results/' + folder + "/simX_phase0.npy", 'wb') as f:
#     np.save(f, simX)
# with open('results/' + folder + "/simU_phase0.npy", 'wb') as f:
#     np.save(f, simU)
# with open('results/' + folder + "/simX_horizon_phase0.npy", 'wb') as f:
#     np.save(f, simX_horizon)
# with open('results/' + folder + "/t_phase0.npy", 'wb') as f:
#     np.save(f, t)

ext_state = get_extended_state(simX, Rw)
plotRes(ext_state,t)
plt.savefig('results/' + folder + "/state_plots_phase0.eps")
plt.savefig('results/' + folder + "/state_plots_phase0.png", dpi=300)

ext_U = get_extended_controls(simU)
plotU(ext_U,t)
plt.savefig('results/' + folder + "/control_plots_phase0.eps")
plt.savefig('results/' + folder + "/control_plots_phase0.png", dpi=300)
...
# set intial condition
v0 = 0
#starting_pose = [ 1.22500000e+00,  6.27935661e-01, -9.05941988e-14,  5.28748433e+00]
starting_pose = stop_state#[ 7.83529412e+00,  8.83362051e-01, -1.36765599e-13,  1.91610636e+00]
x0 = np.array([starting_pose[0]*Rw, Rw, starting_pose[0], 0.5, -90*np.pi/180+starting_pose[1], v0, 0, v0/Rw, 0, starting_pose[3]])
ocp.constraints.x0 = x0
...
t = np.linspace(0.0, Nsim * Tf / N, Nsim)+t_stop
...
plt.savefig('results/' + folder + "/state_plots_total.eps")
plt.savefig('results/' + folder + "/state_plots_total.png", dpi=300)

plotU(np.concatenate((ext_U, simU)),np.concatenate((t_phase0,t)))
plt.savefig('results/' + folder + "/control_plots_total.eps")
plt.savefig('results/' + folder + "/control_plots_total.png", dpi=300)
...
#os.system(f"ffmpeg -f concat -safe 0 -i list.txt -c copy output.mp4")
